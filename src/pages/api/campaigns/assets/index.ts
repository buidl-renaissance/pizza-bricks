...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const rows = typeof campaignId === 'string'
    ? await db.select().from(campaignAssets).where(eq(campaignAssets.campaignId, campaignId)).orderBy(desc(campaignAssets.createdAt)).limit(Number(limit))
    : await db.select().from(campaignAssets).orderBy(desc(campaignAssets.createdAt)).limit(Number(limit));
  ...
  const assets = rows.map(r => ({ ...r, campaignName: campaignMap[r.campaignId] ?? r.campaignId }));
